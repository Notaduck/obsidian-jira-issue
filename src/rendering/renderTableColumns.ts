...
function createNoteLink(issue: IJiraIssue): string {
  issue.fields.description

  const baseFolder = "JIRA"
  const key = issue.key ? "/" + issue.key : ""
  const project = issue.fields.project.name
    ? "/" + issue.fields.project.name
    : ""

  return baseFolder + project + key
}
    ...
    switch (column.type) {
      case ESearchColumnsTypes.KEY:
        createEl("a", {
          ...
          title: column.compact
            ? issue.key
            : RC.issueUrl(issue.account, issue.key),
          parent: createEl("td", { parent: row }),
        })
        break
      case ESearchColumnsTypes.SUMMARY:
        if (column.compact) {
          let summaryCompact = issue.fields.summary.substring(
            0,
            DESCRIPTION_COMPACT_MAX_LENGTH,
          )
      ...
      case ESearchColumnsTypes.DESCRIPTION:
        if (column.compact) {
          let descriptionCompact = issue.fields.description.substring(
            0,
            DESCRIPTION_COMPACT_MAX_LENGTH,
          )
          if (
            issue.fields.description.length > DESCRIPTION_COMPACT_MAX_LENGTH
          ) {
            descriptionCompact += "…"
          }
          createEl("td", {
            text: descriptionCompact,
      ...
      case ESearchColumnsTypes.TYPE:
        ...
        if (issue.fields.issuetype.iconUrl) {
          ...
          if (column.compact) {
            createSpan({
              text: issue.fields.issuetype.name[0].toUpperCase(),
      ...
      case ESearchColumnsTypes.CREATED:
        if (column.compact) {
          createEl("td", {
            text: "🕑",
            title: dateToStr(issue.fields.created),
            parent: row,
          })
        } else {
          createEl("td", {
            text: dateToStr(issue.fields.created),
            parent: row,
          })
        }
        break
      case ESearchColumnsTypes.UPDATED:
        if (column.compact) {
          createEl("td", {
            text: "🕑",
            title: dateToStr(issue.fields.updated),
            parent: row,
          })
        } else {
          createEl("td", {
            text: dateToStr(issue.fields.updated),
            parent: row,
          })
        }
        break
      case ESearchColumnsTypes.REPORTER:
        ...
        if (
          ...
          createEl("img", {
            attr: {
              src: issue.fields.reporter.avatarUrls[AVATAR_RESOLUTION],
              alt: reporterName,
            },
            title: reporterName,
            cls: "avatar-image",
      ...
      case ESearchColumnsTypes.ASSIGNEE:
        ...
        if (
          ...
          createEl("img", {
            attr: {
              src: issue.fields.assignee.avatarUrls[AVATAR_RESOLUTION],
              alt: assigneeName,
            },
            title: assigneeName,
            cls: "avatar-image",
      ...
      case ESearchColumnsTypes.PRIORITY:
        ...
        if (issue.fields.priority && issue.fields.priority.name) {
          if (issue.fields.priority.iconUrl) {
            ...
            createSpan({
              text: issue.fields.priority.name[0].toUpperCase(),
      ...
      case ESearchColumnsTypes.DUE_DATE:
        if (column.compact) {
          createEl("td", {
            text: "🕑",
            title: dateToStr(issue.fields.duedate),
            parent: row,
          })
        } else {
          createEl("td", {
            text: dateToStr(issue.fields.duedate),
            parent: row,
          })
        }
        break
      ...
      case ESearchColumnsTypes.RESOLUTION_DATE:
        if (column.compact) {
          createEl("td", {
            text: "🕑",
            title: dateToStr(issue.fields.resolutiondate),
            parent: row,
          })
        } else {
          createEl("td", {
            text: dateToStr(issue.fields.resolutiondate),
            parent: row,
          })
        }
        break
      case ESearchColumnsTypes.ENVIRONMENT:
        if (column.compact) {
          let environmentCompact = issue.fields.environment.substring(
            0,
            DESCRIPTION_COMPACT_MAX_LENGTH,
          )
          if (
            issue.fields.environment.length > DESCRIPTION_COMPACT_MAX_LENGTH
          ) {
            environmentCompact += "…"
          }
          createEl("td", {
            text: environmentCompact,
            title: issue.fields.environment,
            parent: row,
          })
        } else {
      ...
      case ESearchColumnsTypes.AGGREGATE_TIME_ESTIMATED:
        createEl("td", {
          text: deltaToStr(issue.fields.aggregatetimeestimate),
          parent: row,
        })
        break
      case ESearchColumnsTypes.AGGREGATE_TIME_ORIGINAL_ESTIMATE:
        createEl("td", {
          text: deltaToStr(issue.fields.aggregatetimeoriginalestimate),
          parent: row,
        })
        break
      case ESearchColumnsTypes.AGGREGATE_TIME_SPENT:
        createEl("td", {
          text: deltaToStr(issue.fields.aggregatetimespent),
          parent: row,
        })
        break
      case ESearchColumnsTypes.TIME_ESTIMATE:
        createEl("td", {
          text: deltaToStr(issue.fields.timeestimate),
          parent: row,
        })
        break
      case ESearchColumnsTypes.TIME_ORIGINAL_ESTIMATE:
        createEl("td", {
          text: deltaToStr(issue.fields.timeoriginalestimate),
          parent: row,
        })
        break
      case ESearchColumnsTypes.TIME_SPENT:
        createEl("td", {
          text: deltaToStr(issue.fields.timespent),
          parent: row,
        })
        break
      case ESearchColumnsTypes.AGGREGATE_PROGRESS:
        createEl("td", {
          text: issue.fields.aggregateprogress.percent.toString() + "%",
          parent: row,
        })
        break
      case ESearchColumnsTypes.PROGRESS:
        createEl("td", {
          text: issue.fields.progress.percent.toString() + "%",
          parent: row,
        })
        break
      ...
      case ESearchColumnsTypes.LAST_VIEWED:
        if (column.compact) {
          createEl("td", {
            text: "🕑",
            title: dateToStr(issue.fields.lastViewed),
            parent: row,
          })
        } else {
          createEl("td", {
            text: dateToStr(issue.fields.lastViewed),
            parent: row,
          })
        }
        break
      case ESearchColumnsTypes.DEV_STATUS:
        ...
        if (devStatusCacheItem) {
          ...
          devStatus = await JiraClient.getDevStatus(issue.id, {
        ...
        if (
          prDetails.openCount +
          prDetails.mergedCount +
          prDetails.declinedCount >
          0
        ) {
          if (prDetails.openCount > 0) {
            const prOpen = createSpan({
              parent: cell,
              cls: `pull-request-tag pull-request-open ${RC.getTheme()}`,
          ...
          if (prDetails.mergedCount > 0) {
            const prMerged = createSpan({
              parent: cell,
              cls: `pull-request-tag pull-request-merged ${RC.getTheme()}`,
          ...
          if (prDetails.declinedCount > 0) {
            const prDeclined = createSpan({
              parent: cell,
              cls: `pull-request-tag pull-request-delete ${RC.getTheme()}`,
              title: "Declined pull-request",
            })
